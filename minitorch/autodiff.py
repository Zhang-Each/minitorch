...
class FunctionBase:
    ...
    def apply(cls, *vals):
        ...
        assert isinstance(c, cls.data_type), "Expected return typ %s got %s %s" % (
            cls.data_type,
            type(c),
            c
        )
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        diffs = wrap_tuple(cls.backward(ctx, d_output))
        result = []
        for i in range(len(inputs)):
            if not is_constant(inputs[i]):
                result.append((inputs[i], diffs[i]))
        return result
...
def topological_sort(variable):
    ...
    visited = set()
    def dfs(variable):
        if variable.unique_id in visited or is_constant(variable):
            return
        if not variable.is_leaf():
            for child in variable.history.inputs:
...
def backpropagate(variable, deriv):
    ...
    derivate_dict = {}
    derivate_dict[variable.unique_id] = deriv
    for node in topological_order:
        if node.is_leaf():
            node.accumulate_derivative(derivate_dict[node.unique_id])
        else:
            derivs = node.history.backprop_step(derivate_dict[node.unique_id])
            for tuple in derivs:
                var, der = tuple[0], tuple[1]
                if var.unique_id in derivate_dict.keys():
                    derivate_dict[var.unique_id] += der
                else:
                    derivate_dict[var.unique_id] = der
