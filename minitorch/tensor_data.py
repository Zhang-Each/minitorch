...
def shape_broadcast(shape1, shape2):
    ...
    """
    # TODO: Implement for Task 2.2.
    s1, s2, s = list(shape1), list(shape2), []
    s_len = max(len(s1), len(s2))
    s1 = [1] * (s_len - len(s1)) + s1
    s2 = [1] * (s_len - len(s2)) + s2
    for i in range(s_len):
        if s1[i] % s2[i] == 0 or s2[i] % s1[i] == 0:
            s.append(max(s1[i], s2[i]))
        else:
            raise IndexingError
    return tuple(s)
